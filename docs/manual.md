...
- `編集`: コメント本文を編集します。
- `削除`: 確認後にコメントを一覧と本文上の表示から削除します。
- 返信欄: コメントに返事を追加します。返信にも `編集` と `削除` を利用できます。

`編集` と `削除` は、自分が作成したコメントまたは返信にのみ表示されます。
編集したコメントや返信は `編集済み` と最新の編集日時が表示されます。

コメント位置が本文変更でずれた場合は、対象テキスト、前後文脈、見出し情報から再検出します。完全に追跡できない場合も、コメントは削除されず `位置不明` として残ります。

GUI から編集・削除したコメントや返信は JSON から物理更新・物理削除せず、編集イベントや削除イベントとして保存されます。共有フォルダや Git 同期でも、変更状態を他の利用者へ伝えられます。
