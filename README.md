...
1. このリポジトリを取得するか、tag を指定してローカル版のファイルを取得します。
...
curl -fL -o index.html \
  https://raw.githubusercontent.com/suzuyu/local-markdown-reviewer/${VERSION}/index.html
curl -fL -o app.js \
  https://raw.githubusercontent.com/suzuyu/local-markdown-reviewer/${VERSION}/app.js
curl -fL -o styles.css \
  https://raw.githubusercontent.com/suzuyu/local-markdown-reviewer/${VERSION}/styles.css
```
...
スクリーンショット付きの詳しい操作手順は [操作マニュアル](docs/manual.md) を参照してください。
